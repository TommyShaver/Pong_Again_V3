...
public class SceneLoader : MonoBehaviour
{
    ...
    {
        StartCoroutine(TitleSceneSwitch(4));
    }

    private IEnumerator TitleSceneSwitch(int _seconds)
    {
        Debug.Log("We made it into the Coroutine");
        int j = 0;
        while (j < _seconds)
        {
            Debug.Log(j + "Why no count?");
            if (j == 0)
            {
                //Start animation in global info scene.
                SceneAnimScript._iSceneAnimScript.StartAnimationFadeIn();
