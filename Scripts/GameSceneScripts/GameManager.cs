...
public class GameManager : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        SceneAnimScript._iSceneAnimScript.SetToDefault();
    }
    ...
    public void WhichGoalScored(bool leftGoal)
    {
        if (_playerOneScore != 10 && _playerTwoScore != 10)
        {
            ...
            if (leftGoal == true)
            {
                ...
                _isLeftGoal = true;
                StartCoroutine(WaitForReset(5));
                SFXManager._iSFXManager.GoalHorn();
            }
            else
            {
    ...
    private void PauseTheGame()
    {
        if(_gamePaused == false)
        {
            ...
            Cursor.visible = true;
            SFXManager._iSFXManager.PauseSFX(true);
        }
        else
        {
            Time.timeScale = 1;
            ...
            Cursor.visible = true;
            SFXManager._iSFXManager.PauseSFX(false);
        }
    }
    ...
    private IEnumerator WaitForReset(int _timer)
    {
        ...
        while (i < _timer)
        {
            ...
            if(i == 3)
            {
                SFXManager._iSFXManager.WhistleStart();
            }
            if (i == 4)
            {
                //Start of next play.
    ...
    //Back to title scene ======================================
    public void BackToTitleScreenGameManager()
    {
        SceneLoader._isceneLoader.BacktoTitleScreen();
        //If this isn't set here then it will never count up
        //This need to be its own manager or scene
        //Time does travel between scenes.
        Time.timeScale = 1;
    }
}
